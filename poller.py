...
CLIENT_CLOCK_SKEW_MS = 5 * 60 * 1000
MAX_EXTENSION_WINDOW_MS = 24 * 60 * 60 * 1000
        ...
        for pk, label in ports.items():
            ...
            if isinstance(reset, dict):
                ...
                if (
                    ...
                    and reset_ms <= now_ms + CLIENT_CLOCK_SKEW_MS
...
def _policy_deadline_ms(
    ...
    if isinstance(ext, dict):
        ...
        max_until_ms = max(base, now_ms) + MAX_EXTENSION_WINDOW_MS
        if (
            until_ms is not None
            and until_ms >= base
            and until_ms <= max_until_ms
        ):
            return until_ms
    return base
...
def preserve_station_state_on_fetch_errors(
    ...
    """
    A transient ChargePoint fetch failure should not erase the last known port
    session state when /stations is replaced as a whole.
    """
    prev_root = prev_root or {}
