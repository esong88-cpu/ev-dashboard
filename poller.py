...
MAX_RESET_AGE_MS = 24 * 60 * 60 * 1000
...
def enrich_stations_with_port_sessions(
    ...
    for sid, entry in list(stations.items()):
        ...
        for pk, label in ports.items():
            ...
            reset_floor_ms = old_start_ms if old_start_ms > 0 else (0 if was_occ else started_ms)
            if reset_ms is not None and reset_ms > reset_floor_ms:
                started_at = datetime.fromtimestamp(reset_ms / 1000, tz=timezone.utc).isoformat()
                ext_clear.append(f"{sid}-{pk_s}")
...
def _valid_reset_ms(reset: Any, now_ms: int) -> Optional[int]:
    if not isinstance(reset, dict):
        return None
    reset_ms = _finite_ms(reset.get("at_ms"))
    if reset_ms is None or reset_ms <= 0:
        return None
    if reset_ms > now_ms + CLOCK_SKEW_MS:
        return None
    if reset_ms < now_ms - MAX_RESET_AGE_MS:
        return None
    return reset_ms
...
def preserve_station_state_on_fetch_errors(
    ...
    for sid, entry in list(stations.items()):
        ...
        preserved["last_fetch_error"] = str(entry.get("error") or "")
