...
def enrich_policy_complete_since(
    ...
    """
    prev_stations: Dict[str, Any] = _station_entries(prev_root or {})
    now_iso = datetime.now(timezone.utc).isoformat()
    now_ms = int(datetime.now(timezone.utc).timestamp() * 1000)
...
def preserve_previous_station_on_errors(
    ...
    """
    Keep the last good station snapshot when a transient fetch fails.

    The poller replaces /stations wholesale, so writing an error-only object would
    otherwise delete ports and session clocks, resetting policy timers on recovery.
    """
    prev_stations = _station_entries(prev_root or {})
    for sid, entry in list(stations.items()):
        ...
        preserved["last_fetch_error"] = str(entry.get("error") or "Unknown fetch error")
