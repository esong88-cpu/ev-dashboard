...
def _coerce_finite_ms(value: Any) -> int | None:
    ...
    for sid, entry in list(stations.items()):
        ...
        for pk, label in ports.items():
            ...
            if not was_occ or not old_start:
                ...
                old_start_ms = _iso_to_utc_ms(str(old_start))
                if old_start_ms <= 0:
                    started_at = now_iso
                    started_ms = now_ms
                else:
            ...
            if isinstance(reset, dict):
                reset_ms = _coerce_finite_ms(reset.get("at_ms"))
                if (
                    reset_ms is not None
                    and reset_ms > started_ms
                    and reset_ms <= now_ms + MAX_RESET_FUTURE_SKEW_MS
                ):
                    started_at = _iso_from_ms(reset_ms)
                    started_ms = reset_ms
...
def _policy_deadline_ms(
    ...
    if isinstance(ext, dict):
        um = _coerce_finite_ms(ext.get("until_ms"))
        if um is not None and base <= um <= base + MAX_EXTENSION_MS:
            return um
    return base
...
def preserve_station_state_on_fetch_errors(
    ...
    """
    Keep the last known ports/session clocks when a poll returns an error or an
    empty port map; otherwise a transient ChargePoint issue erases active timers.
    """
    prev_stations: Dict[str, Any] = {
        k: v
        for k, v in (prev_root or {}).items()
    ...
    for sid, entry in list(stations.items()):
        if not isinstance(entry, dict):
            continue
        ports = _rtdb_map(entry.get("ports"))
        fetch_error = str(entry.get("error") or "").strip()
        if ports and not fetch_error:
            continue
        ...
        if fetch_error:
            preserved["last_fetch_error"] = fetch_error
        else:
            preserved["last_fetch_error"] = "Station returned no ports"
